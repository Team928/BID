...
const WroteReviews = () => {
  const { useWorteReviewList } = useReview();
  const { data: reviewInfo } = useWorteReviewList();
    ...
    <div>
      {reviewCnt === 0 ? (
        <div className="w-full h-[calc(100vh-64px)] flex justify-center items-center flex-col">
          ...
          <div className="pt-2">아직 작성한 리뷰가 없어요</div>
        </div>
      ) : (
        <div>
          <div className="px-6 ">
                ...
                <div className="flex-1 flex flex-col gap-1 px-2">
                  <div className="flex items-center justify-between pr-3">
                    <div className="flex items-center gap-3">
                      <p className="text-md font-bold">{item.tergetNickname}</p>
